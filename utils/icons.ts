const iconsSchema = {
  iconDefinitions: {
    _file: { iconPath: '/images/icons/file.svg' },
    _file_light: { iconPath: '/images/icons/file_light.svg' },
    _folder: { iconPath: '/images/icons/folder.svg' },
    _folder_light: { iconPath: '/images/icons/folder_light.svg' },
    _folder_open: { iconPath: '/images/icons/folder_open.svg' },
    _folder_light_open: { iconPath: '/images/icons/folder_light_open.svg' },
    _root_folder: { iconPath: '/images/icons/root_folder.svg' },
    _root_folder_open: { iconPath: '/images/icons/root_folder_open.svg' },
    _root_folder_light: { iconPath: '/images/icons/root_folder_light.svg' },
    _root_folder_light_open: {
      iconPath: '/images/icons/root_folder_light_open.svg',
    },
    _f_elm: { iconPath: '/images/icons/elm.svg' },
    _f_sass: { iconPath: '/images/icons/sass.svg' },
    _f_scss: { iconPath: '/images/icons/scss.svg' },
    _f_less: { iconPath: '/images/icons/less.svg' },
    _f_stylus: { iconPath: '/images/icons/stylus.svg' },
    _f_postcssconfig: { iconPath: '/images/icons/postcssconfig.svg' },
    _f_docker: { iconPath: '/images/icons/docker.svg' },
    _f_dockerignore: { iconPath: '/images/icons/dockerignore.svg' },
    _f_dockerdebug: { iconPath: '/images/icons/dockerdebug.svg' },
    _f_git: { iconPath: '/images/icons/git.svg' },
    _f_readme: { iconPath: '/images/icons/readme.svg' },
    _f_html: { iconPath: '/images/icons/html.svg' },
    _f_json: { iconPath: '/images/icons/json.svg' },
    _f_rjson: { iconPath: '/images/icons/rjson.svg' },
    _f_markdown: { iconPath: '/images/icons/markdown.svg' },
    _f_css: { iconPath: '/images/icons/css.svg' },
    _f_cssmap: { iconPath: '/images/icons/cssmap.svg' },
    _f_js: { iconPath: '/images/icons/js.svg' },
    _f_jsmap: { iconPath: '/images/icons/jsmap.svg' },
    _f_typescript: { iconPath: '/images/icons/typescript.svg' },
    _f_typescriptdef: { iconPath: '/images/icons/typescriptdef.svg' },
    _f_tsx: { iconPath: '/images/icons/tsx.svg' },
    _f_vue: { iconPath: '/images/icons/vue.svg' },
    _f_image: { iconPath: '/images/icons/image.svg' },
    _f_zip: { iconPath: '/images/icons/zip.svg' },
    _f_yarn: { iconPath: '/images/icons/yarn.svg' },
    _f_yarnerror: { iconPath: '/images/icons/yarnerror.svg' },
    _f_error: { iconPath: '/images/icons/error.svg' },
    _f_npm: { iconPath: '/images/icons/npm.svg' },
    _f_npmlock: { iconPath: '/images/icons/npmlock.svg' },
    _f_svg: { iconPath: '/images/icons/svg.svg' },
    _f_yaml: { iconPath: '/images/icons/yaml.svg' },
    _f_fontwoff2: { iconPath: '/images/icons/fontwoff2.svg' },
    _f_fontwoff: { iconPath: '/images/icons/fontwoff.svg' },
    _f_fontttf: { iconPath: '/images/icons/fontttf.svg' },
    _f_fontotf: { iconPath: '/images/icons/fontotf.svg' },
    _f_php: { iconPath: '/images/icons/php.svg' },
    _f_twig: { iconPath: '/images/icons/twig.svg' },
    _f_composer: { iconPath: '/images/icons/composer.svg' },
    _f_composerlock: { iconPath: '/images/icons/composerlock.svg' },
    _f_symfony: { iconPath: '/images/icons/symfony.svg' },
    _f_xml: { iconPath: '/images/icons/xml.svg' },
    _f_robots: { iconPath: '/images/icons/robots.svg' },
    _f_bundler: { iconPath: '/images/icons/bundler.svg' },
    _f_stylelint: { iconPath: '/images/icons/stylelint.svg' },
    _f_prettier: { iconPath: '/images/icons/prettier.svg' },
    _f_prettierignore: { iconPath: '/images/icons/prettierignore.svg' },
    _f_python: { iconPath: '/images/icons/python.svg' },
    _f_log: { iconPath: '/images/icons/log.svg' },
    _f_license: { iconPath: '/images/icons/license.svg' },
    _f_audio: { iconPath: '/images/icons/audio.svg' },
    _f_audiomp3: { iconPath: '/images/icons/audiomp3.svg' },
    _f_audioogg: { iconPath: '/images/icons/audioogg.svg' },
    _f_audiowav: { iconPath: '/images/icons/audiowav.svg' },
    _f_lua: { iconPath: '/images/icons/lua.svg' },
    _f_csharp: { iconPath: '/images/icons/csharp.svg' },
    _f_dartlang: { iconPath: '/images/icons/dartlang.svg' },
    _f_eslint: { iconPath: '/images/icons/eslint.svg' },
    _f_handlebars: { iconPath: '/images/icons/handlebars.svg' },
    _f_webpack: { iconPath: '/images/icons/webpack.svg' },
    _f_ruby: { iconPath: '/images/icons/ruby.svg' },
    _f_erb: { iconPath: '/images/icons/erb.svg' },
    _f_jest: { iconPath: '/images/icons/jest.svg' },
    _f_babel: { iconPath: '/images/icons/babel.svg' },
    _f_toml: { iconPath: '/images/icons/toml.svg' },
    _f_elixir: { iconPath: '/images/icons/elixir.svg' },
    _f_exs: { iconPath: '/images/icons/exs.svg' },
    _f_rust: { iconPath: '/images/icons/rust.svg' },
    _f_rustfmt: { iconPath: '/images/icons/rustfmt.svg' },
    _f_reactjs: { iconPath: '/images/icons/reactjs.svg' },
    _f_reactts: { iconPath: '/images/icons/reactts.svg' },
    _f_angular: { iconPath: '/images/icons/angular.svg' },
    _f_rollup: { iconPath: '/images/icons/rollup.svg' },
    _f_java: { iconPath: '/images/icons/java.svg' },
    _f_gulp: { iconPath: '/images/icons/gulp.svg' },
    _f_grunt: { iconPath: '/images/icons/grunt.svg' },
    _f_editorconfig: { iconPath: '/images/icons/editorconfig.svg' },
    _f_light_editorconfig: {
      iconPath: '/images/icons/light_editorconfig.svg',
    },
    _f_shell: { iconPath: '/images/icons/shell.svg' },
    _f_vscode: { iconPath: '/images/icons/vscode.svg' },
    _f_ai: { iconPath: '/images/icons/ai.svg' },
    _f_photoshop: { iconPath: '/images/icons/photoshop.svg' },
    _f_pdf: { iconPath: '/images/icons/pdf.svg' },
    _f_go: { iconPath: '/images/icons/go.svg' },
    _f_go_package: { iconPath: '/images/icons/go_package.svg' },
    _f_tmpl: { iconPath: '/images/icons/tmpl.svg' },
    _f_svelte: { iconPath: '/images/icons/svelte.svg' },
    _f_browserslist: { iconPath: '/images/icons/browserslist.svg' },
    _f_erlang: { iconPath: '/images/icons/erlang.svg' },
    _f_vite: { iconPath: '/images/icons/vite.svg' },
    _f_mint: { iconPath: '/images/icons/mint.svg' },
    _f_eex: { iconPath: '/images/icons/eex.svg' },
    _f_env: { iconPath: '/images/icons/env.svg' },
    _f_nvm: { iconPath: '/images/icons/nvm.svg' },
    _f_sql: { iconPath: '/images/icons/sql.svg' },
    _f_nginx: { iconPath: '/images/icons/nginx.svg' },
    _f_conf: { iconPath: '/images/icons/conf.svg' },
    _f_c: { iconPath: '/images/icons/c.svg' },
    _f_cpp: { iconPath: '/images/icons/cpp.svg' },
    _f_hpp: { iconPath: '/images/icons/hpp.svg' },
    _f_powershell: { iconPath: '/images/icons/powershell.svg' },
    _f_powershelldata: { iconPath: '/images/icons/powershelldata.svg' },
    _f_powershellmodule: { iconPath: '/images/icons/powershellmodule.svg' },
    _f_njk: { iconPath: '/images/icons/njk.svg' },
    _f_blade: { iconPath: '/images/icons/blade.svg' },
    _f_liquid: { iconPath: '/images/icons/liquid.svg' },
    _f_pug: { iconPath: '/images/icons/pug.svg' },
    _f_cheader: { iconPath: '/images/icons/cheader.svg' },
    _f_m: { iconPath: '/images/icons/m.svg' },
    _f_swift: { iconPath: '/images/icons/swift.svg' },
    _f_graphql: { iconPath: '/images/icons/graphql.svg' },
    _f_groovy: { iconPath: '/images/icons/groovy.svg' },
    _f_testjs: { iconPath: '/images/icons/testjs.svg' },
    _f_testts: { iconPath: '/images/icons/testts.svg' },
    _f_storybook: { iconPath: '/images/icons/storybook.svg' },
    _f_wasm: { iconPath: '/images/icons/wasm.svg' },
    _f_imagepng: { iconPath: '/images/icons/imagepng.svg' },
    _f_imagejpg: { iconPath: '/images/icons/imagejpg.svg' },
    _f_imagegif: { iconPath: '/images/icons/imagegif.svg' },
    _f_imagewebp: { iconPath: '/images/icons/imagewebp.svg' },
    _f_imageico: { iconPath: '/images/icons/imageico.svg' },
    _f_cli: { iconPath: '/images/icons/cli.svg' },
    _f_bat: { iconPath: '/images/icons/bat.svg' },
    _f_nuxt: { iconPath: '/images/icons/nuxt.svg' },
    _f_netlify: { iconPath: '/images/icons/netlify.svg' },
    _f_vercel: { iconPath: '/images/icons/vercel.svg' },
    _f_tailwind: { iconPath: '/images/icons/tailwind.svg' },
    _f_windi: { iconPath: '/images/icons/windi.svg' },
    _f_nodemon: { iconPath: '/images/icons/nodemon.svg' },
    _f_jinja: { iconPath: '/images/icons/jinja.svg' },
    _f_haml: { iconPath: '/images/icons/haml.svg' },
    _f_smarty: { iconPath: '/images/icons/smarty.svg' },
    _f_haskell: { iconPath: '/images/icons/haskell.svg' },
    _f_gradle: { iconPath: '/images/icons/gradle.svg' },
    _f_nim: { iconPath: '/images/icons/nim.svg' },
    _f_nestjs: { iconPath: '/images/icons/nestjs.svg' },
    _f_nestjscontroller: { iconPath: '/images/icons/nestjscontroller.svg' },
    _f_nestjsservice: { iconPath: '/images/icons/nestjsservice.svg' },
    _f_nestjsmodule: { iconPath: '/images/icons/nestjsmodule.svg' },
    _f_nestjsguard: { iconPath: '/images/icons/nestjsguard.svg' },
    _f_nestjsdecorator: { iconPath: '/images/icons/nestjsdecorator.svg' },
    _f_nestjsfilter: { iconPath: '/images/icons/nestjsfilter.svg' },
    _f_jenkins: { iconPath: '/images/icons/jenkins.svg' },
    _f_txt: { iconPath: '/images/icons/txt.svg' },
    _f_binary: { iconPath: '/images/icons/binary.svg' },
    _f_cert: { iconPath: '/images/icons/cert.svg' },
    _f_key: { iconPath: '/images/icons/key.svg' },
    _f_prisma: { iconPath: '/images/icons/prisma.svg' },
    _f_compodoc: { iconPath: '/images/icons/compodoc.svg' },
    _f_julia: { iconPath: '/images/icons/julia.svg' },
    _f_csv: { iconPath: '/images/icons/csv.svg' },
    _f_karma: { iconPath: '/images/icons/karma.svg' },
    _f_tsconfig: { iconPath: '/images/icons/tsconfig.svg' },
    _f_travis: { iconPath: '/images/icons/travis.svg' },
    _f_razor: { iconPath: '/images/icons/razor.svg' },
    _f_cshtml: { iconPath: '/images/icons/cshtml.svg' },
    _f_jsp: { iconPath: '/images/icons/jsp.svg' },
    _f_avi: { iconPath: '/images/icons/avi.svg' },
    _f_mp4: { iconPath: '/images/icons/mp4.svg' },
    _f_mov: { iconPath: '/images/icons/mov.svg' },
    _f_video: { iconPath: '/images/icons/video.svg' },
    _f_rescript: { iconPath: '/images/icons/rescript.svg' },
    _f_bsconfig: { iconPath: '/images/icons/bsconfig.svg' },
    _f_opengl: { iconPath: '/images/icons/opengl.svg' },
    _f_llvm: { iconPath: '/images/icons/llvm.svg' },
    _f_info: { iconPath: '/images/icons/info.svg' },
    _f_ipynb: { iconPath: '/images/icons/ipynb.svg' },
    _f_sol: { iconPath: '/images/icons/sol.svg' },
    _f_bicep: { iconPath: '/images/icons/bicep.svg' },
    _f_qt: { iconPath: '/images/icons/qt.svg' },
    _f_ui: { iconPath: '/images/icons/ui.svg' },
    _f_prolog: { iconPath: '/images/icons/prolog.svg' },
    _f_user: { iconPath: '/images/icons/user.svg' },
    _f_clojure: { iconPath: '/images/icons/clojure.svg' },
    _f_terraform: { iconPath: '/images/icons/terraform.svg' },
    _f_terraformvars: { iconPath: '/images/icons/terraformvars.svg' },
    _f_parcel: { iconPath: '/images/icons/parcel.svg' },
    _f_ocaml: { iconPath: '/images/icons/ocaml.svg' },
    _f_ocamli: { iconPath: '/images/icons/ocamli.svg' },
    _f_ocamly: { iconPath: '/images/icons/ocamly.svg' },
    _f_ocamll: { iconPath: '/images/icons/ocamll.svg' },
    _f_fsharp: { iconPath: '/images/icons/fsharp.svg' },
    _f_dune: { iconPath: '/images/icons/dune.svg' },
    _f_duneproject: { iconPath: '/images/icons/duneproject.svg' },
    _f_adonis: { iconPath: '/images/icons/adonis.svg' },
    _f_adonisconfig: { iconPath: '/images/icons/adonisconfig.svg' },
    _f_astro: { iconPath: '/images/icons/astro.svg' },
    _f_astroconfig: { iconPath: '/images/icons/astroconfig.svg' },
    _f_godot: { iconPath: '/images/icons/godot.svg' },
    _f_tscn: { iconPath: '/images/icons/tscn.svg' },
    _f_svelteconfig: { iconPath: '/images/icons/svelteconfig.svg' },
    _f_toolversions: { iconPath: '/images/icons/toolversions.svg' },
    _f_cmake: { iconPath: '/images/icons/cmake.svg' },
    _f_conan: { iconPath: '/images/icons/conan.svg' },
    _f_cargo: { iconPath: '/images/icons/cargo.svg' },
    _f_cargolock: { iconPath: '/images/icons/cargolock.svg' },
    _f_pnpm: { iconPath: '/images/icons/pnpm.svg' },
    _f_pnpmlock: { iconPath: '/images/icons/pnpmlock.svg' },
    _f_tauri: { iconPath: '/images/icons/tauri.svg' },
    _f_todo: { iconPath: '/images/icons/todo.svg' },
    _f_nextconfig: { iconPath: '/images/icons/nextconfig.svg' },
    _f_nextron: { iconPath: '/images/icons/nextron.svg' },
    _f_hashicorp: { iconPath: '/images/icons/hashicorp.svg' },
    _f_sentinel: { iconPath: '/images/icons/sentinel.svg' },
    _f_poetry: { iconPath: '/images/icons/poetry.svg' },
    _f_poetrylock: { iconPath: '/images/icons/poetrylock.svg' },
    _f_pyproject: { iconPath: '/images/icons/pyproject.svg' },
    _f_cucumber: { iconPath: '/images/icons/cucumber.svg' },
    _f_nix: { iconPath: '/images/icons/nix.svg' },
    _f_flakelock: { iconPath: '/images/icons/flakelock.svg' },
    _f_edge: { iconPath: '/images/icons/edge.svg' },
    _f_ace: { iconPath: '/images/icons/ace.svg' },
    _f_acemanifest: { iconPath: '/images/icons/acemanifest.svg' },
    _f_sqlite: { iconPath: '/images/icons/sqlite.svg' },
    _f_knex: { iconPath: '/images/icons/knex.svg' },
    _f_launch: { iconPath: '/images/icons/launch.svg' },
    _f_redis: { iconPath: '/images/icons/redis.svg' },
    _f_nvim: { iconPath: '/images/icons/nvim.svg' },
    _f_sequelize: { iconPath: '/images/icons/sequelize.svg' },
    _f_tres: { iconPath: '/images/icons/tres.svg' },
    _f_sln: { iconPath: '/images/icons/sln.svg' },
    _f_suo: { iconPath: '/images/icons/suo.svg' },
    _f_csproj: { iconPath: '/images/icons/csproj.svg' },
    _f_mesh: { iconPath: '/images/icons/mesh.svg' },
    _f_gbl: { iconPath: '/images/icons/gbl.svg' },
    _f_cypress: { iconPath: '/images/icons/cypress.svg' },
    _f_playright: { iconPath: '/images/icons/playright.svg' },
    _f_vitest: { iconPath: '/images/icons/vitest.svg' },
    _f_flutter: { iconPath: '/images/icons/flutter.svg' },
    _f_flutterlock: { iconPath: '/images/icons/flutterlock.svg' },
    _f_flutterpackage: { iconPath: '/images/icons/flutterpackage.svg' },
    _f_htaccess: { iconPath: '/images/icons/htaccess.svg' },
    _f_nx: { iconPath: '/images/icons/nx.svg' },
    _f_v: { iconPath: '/images/icons/v.svg' },
    _f_vmod: { iconPath: '/images/icons/vmod.svg' },
    _f_rego: { iconPath: '/images/icons/rego.svg' },
    _f_scala: { iconPath: '/images/icons/scala.svg' },
    _f_d: { iconPath: '/images/icons/d.svg' },
    _f_adoc: { iconPath: '/images/icons/adoc.svg' },
    _f_motoko: { iconPath: '/images/icons/motoko.svg' },
    _f_diff: { iconPath: '/images/icons/diff.svg' },
    _f_quasar: { iconPath: '/images/icons/quasar.svg' },
  },
  fileExtensions: {
    'wma': '_f_audio',
    'wav': '_f_audiowav',
    'vox': '_f_audio',
    'tta': '_f_audio',
    'raw': '_f_audio',
    'ra': '_f_audio',
    'opus': '_f_audio',
    'ogg': '_f_audioogg',
    'oga': '_f_audio',
    'msv': '_f_audio',
    'mpc': '_f_audio',
    'mp3': '_f_audiomp3',
    'mogg': '_f_audio',
    'mmf': '_f_audio',
    'm4p': '_f_audio',
    'm4b': '_f_audio',
    'm4a': '_f_audio',
    'ivs': '_f_audio',
    'iklax': '_f_audio',
    'gsm': '_f_audio',
    'flac': '_f_audio',
    'dvf': '_f_audio',
    'dss': '_f_audio',
    'dct': '_f_audio',
    'au': '_f_audio',
    'ape': '_f_audio',
    'amr': '_f_audio',
    'aiff': '_f_audio',
    'act': '_f_audio',
    'aac': '_f_audio',
    'wmv': '_f_video',
    'webm': '_f_video',
    'vob': '_f_video',
    'svi': '_f_video',
    'rmvb': '_f_video',
    'rm': '_f_video',
    'ogv': '_f_video',
    'nsv': '_f_video',
    'mpv': '_f_video',
    'mpg': '_f_video',
    'mpeg2': '_f_video',
    'mpeg': '_f_video',
    'mpe': '_f_video',
    'mp4': '_f_mp4',
    'mp2': '_f_video',
    'mov': '_f_mov',
    'mk3d': '_f_video',
    'mkv': '_f_video',
    'm4v': '_f_video',
    'm2v': '_f_video',
    'flv': '_f_video',
    'f4v': '_f_video',
    'f4p': '_f_video',
    'f4b': '_f_video',
    'f4a': '_f_video',
    'qt': '_f_video',
    'divx': '_f_video',
    'avi': '_f_video',
    'amv': '_f_video',
    'asf': '_f_video',
    '3gp': '_f_video',
    '3g2': '_f_video',
    'ico': '_f_imageico',
    'tiff': '_f_image',
    'bmp': '_f_image',
    'png': '_f_imagepng',
    'gif': '_f_imagegif',
    'jpg': '_f_imagejpg',
    'jpeg': '_f_image',
    'dmp': '_f_log',
    'woff2': '_f_fontwoff2',
    'woff': '_f_fontwoff',
    'ttf': '_f_fontttf',
    'otf': '_f_fontotf',
    'nim': '_f_nim',
    'hs': '_f_haskell',
    'tpl': '_f_smarty',
    'haml': '_f_haml',
    'cmd': '_f_cli',
    'erb': '_f_erb',
    'm': '_f_m',
    'accdb': '_f_access',
    'accdt': '_f_access',
    'mdb': '_f_access',
    'accda': '_f_access',
    'accdc': '_f_access',
    'accde': '_f_access',
    'accdp': '_f_access',
    'accdr': '_f_access',
    'accdu': '_f_access',
    'ade': '_f_access',
    'adp': '_f_access',
    'laccdb': '_f_access',
    'ldb': '_f_access',
    'mam': '_f_access',
    'maq': '_f_access',
    'mdw': '_f_access',
    'afdesign': '_f_affinitydesigner',
    'affinitydesigner': '_f_affinitydesigner',
    'afphoto': '_f_affinityphoto',
    'affinityphoto': '_f_affinityphoto',
    'afpub': '_f_affinitypublisher',
    'affinitypublisher': '_f_affinitypublisher',
    'ai': '_f_ai',
    'ino': '_f_arduino',
    'pde': '_f_arduino',
    'aspx': '_f_aspx',
    'ascx': '_f_aspx',
    'awk': '_f_awk',
    'a': '_f_binary',
    'app': '_f_binary',
    'bin': '_f_binary',
    'cmo': '_f_binary',
    'cmx': '_f_binary',
    'cma': '_f_binary',
    'cmxa': '_f_binary',
    'cmi': '_f_binary',
    'exe': '_f_binary',
    'dll': '_f_binary',
    'hl': '_f_binary',
    'ilk': '_f_binary',
    'lib': '_f_binary',
    'n': '_f_binary',
    'ndll': '_f_binary',
    'o': '_f_binary',
    'obj': '_f_binary',
    'pyc': '_f_binary',
    'pyd': '_f_binary',
    'pyo': '_f_binary',
    'pdb': '_f_binary',
    'scpt': '_f_binary',
    'scptd': '_f_binary',
    'so': '_f_binary',
    'gemfile': '_f_bundler',
    'gemfile.lock': '_f_bundler',
    'cake': '_f_cake',
    'csr': '_f_cert',
    'crt': '_f_cert',
    'cer': '_f_cert',
    'der': '_f_cert',
    'pfx': '_f_cert',
    'p12': '_f_cert',
    'p7b': '_f_cert',
    'p7r': '_f_cert',
    'src': '_f_cert',
    'crl': '_f_cert',
    'sst': '_f_cert',
    'stl': '_f_cert',
    'lucee': '_f_cf',
    'h': '_f_cheader',
    'class': '_f_class',
    'cjm': '_f_clojure',
    'kit': '_f_codekit',
    'plist': '_f_config',
    'csx': '_f_csharp',
    'cs': '_f_csharp',
    'csproj': '_f_csproj',
    'css': '_f_css',
    'css.map': '_f_cssmap',
    'db': '_f_db',
    'djt': '_f_django',
    'eco': '_f_docpad',
    'drawio': '_f_drawio',
    'dio': '_f_drawio',
    'drawio.png': '_f_drawio',
    'drawio.svg': '_f_drawio',
    'ejs': '_f_ejs',
    'el': '_f_emacs',
    'elc': '_f_emacs',
    'ensime': '_f_ensime',
    'eps': '_f_eps',
    'exs': '_f_exs',
    'xls': '_f_excel',
    'xlsx': '_f_excel',
    'xlsm': '_f_excel',
    'ods': '_f_excel',
    'fods': '_f_excel',
    'fbx': '_f_fbx',
    'fla': '_f_fla',
    'swf': '_f_flash',
    'swc': '_f_flash',
    'js.flow': '_f_flow',
    'fsproj': '_f_fsproj',
    'gmx': '_f_gamemaker',
    'yy': '_f_gamemaker2',
    'yyp': '_f_gamemaker2',
    'gradle': '_f_gradle',
    'hxproj': '_f_haxedevelop',
    'idr': '_f_idris',
    'lidr': '_f_idris',
    'ibc': '_f_idrisbin',
    'ipkg': '_f_idrispkg',
    'inc': '_f_inc',
    'include': '_f_inc',
    'infopathxml': '_f_infopath',
    'xsn': '_f_infopath',
    'xsf': '_f_infopath',
    'xtp2': '_f_infopath',
    'jar': '_f_jar',
    'jbuilder': '_f_jbuilder',
    'js.snap': '_f_jest_snapshot',
    'jsx.snap': '_f_jest_snapshot',
    'ts.snap': '_f_jest_snapshot',
    'tsx.snap': '_f_jest_snapshot',
    'js.map': '_f_jsmap',
    'json5': '_f_json5',
    'jsonld': '_f_jsonld',
    'json-ld': '_f_jsonld',
    'rjson': '_f_rjson',
    'json': '_f_json',
    'jss': '_f_jss',
    'key': '_f_key',
    'pem': '_f_key',
    'master': '_f_layout',
    'layout.html': '_f_layout',
    'layout.htm': '_f_layout',
    'html': '_f_html',
    'enc': '_f_license',
    'hxp': '_f_lime',
    'liquid': '_f_liquid',
    'ls': '_f_livescript',
    'lnk': '_f_lnk',
    'log': '_f_log',
    'tlg': '_f_log',
    'crec': '_f_lync',
    'ocrec': '_f_lync',
    'makefile': '_f_makefile',
    'map': '_f_map',
    'mdown': '_f_markdown',
    'markdown': '_f_markdown',
    'md': '_f_markdown',
    'marko.js': '_f_markojs',
    'fig': '_f_matlab',
    'mex': '_f_matlab',
    'mexn': '_f_matlab',
    'mexrs6': '_f_matlab',
    'mn': '_f_matlab',
    'mum': '_f_matlab',
    'mx': '_f_matlab',
    'mx3': '_f_matlab',
    'rwd': '_f_matlab',
    'slx': '_f_matlab',
    'slddc': '_f_matlab',
    'smv': '_f_matlab',
    'xvc': '_f_matlab',
    'mdx': '_f_mdx',
    'mustache': '_f_mustache',
    'mst': '_f_mustache',
    'njsproj': '_f_njsproj',
    'nupkg': '_f_nuget',
    'nuspec': '_f_nuget',
    'psmdcp': '_f_nuget',
    'nunj': '_f_nunjucks',
    'njs': '_f_nunjucks',
    'one': '_f_onenote',
    'onepkg': '_f_onenote',
    'onetoc': '_f_onenote',
    'onetoc2': '_f_onenote',
    'sig': '_f_onenote',
    'cl': '_f_opencl',
    'opencl': '_f_opencl',
    'org': '_f_org',
    'pst': '_f_outlook',
    'bcmx': '_f_outlook',
    'otm': '_f_outlook',
    'msg': '_f_outlook',
    'oft': '_f_outlook',
    'ovpn': '_f_ovpn',
    'pkg': '_f_package',
    'pcd': '_f_pcl',
    'pdf': '_f_pdf',
    'psd': '_f_photoshop',
    'php1': '_f_php',
    'php2': '_f_php',
    'php3': '_f_php',
    'php4': '_f_php',
    'php5': '_f_php',
    'php6': '_f_php',
    'phps': '_f_php',
    'phpsa': '_f_php',
    'phpt': '_f_php',
    'phtml': '_f_php',
    'phar': '_f_php',
    'pu': '_f_plantuml',
    'plantuml': '_f_plantuml',
    'iuml': '_f_plantuml',
    'puml': '_f_plantuml',
    'pck': '_f_plsql_package',
    'pkb': '_f_plsql_package_body',
    'pkh': '_f_plsql_package_header',
    'pks': '_f_plsql_package_spec',
    'po': '_f_poedit',
    'pot': '_f_powerpoint',
    'potx': '_f_powerpoint',
    'potm': '_f_powerpoint',
    'pps': '_f_powerpoint',
    'ppsx': '_f_powerpoint',
    'ppsm': '_f_powerpoint',
    'ppt': '_f_powerpoint',
    'pptx': '_f_powerpoint',
    'pptm': '_f_powerpoint',
    'pa': '_f_powerpoint',
    'ppa': '_f_powerpoint',
    'ppam': '_f_powerpoint',
    'sldm': '_f_powerpoint',
    'sldx': '_f_powerpoint',
    'format.ps1xml': '_f_powershell_format',
    'types.ps1xml': '_f_powershell_types',
    'pro': '_f_prolog',
    'P': '_f_prolog',
    'pub': '_f_publisher',
    'puz': '_f_publisher',
    'q': '_f_q',
    'qbs': '_f_qbs',
    'qvd': '_f_qlikview',
    'qvw': '_f_qlikview',
    'rake': '_f_rake',
    'rt': '_f_reacttemplate',
    'reg': '_f_registry',
    'rproj': '_f_rproj',
    'sls': '_f_saltstack',
    'sass': '_f_scss',
    'scss': '_f_scss',
    'unity': '_f_shaderlab',
    'fish': '_f_shell',
    'sketch': '_f_sketch',
    'eskip': '_f_skipper',
    'sqlite': '_f_sqlite',
    'sqlite3': '_f_sqlite',
    'db3': '_f_sqlite',
    'sss': '_f_sss',
    'dta': '_f_stata',
    'storyboard': '_f_storyboard',
    'story.js': '_f_storybook',
    'story.jsx': '_f_storybook',
    'story.ts': '_f_storybook',
    'story.tsx': '_f_storybook',
    'stories.js': '_f_storybook',
    'stories.jsx': '_f_storybook',
    'stories.ts': '_f_storybook',
    'stories.tsx': '_f_storybook',
    'svg': '_f_svg',
    'tcl': '_f_tcl',
    'exp': '_f_tcl',
    'tfstate': '_f_terraform',
    'tst': '_f_test',
    'test.js': '_f_testjs',
    'test.jsx': '_f_testjs',
    'test.mjs': '_f_testjs',
    'spec.js': '_f_testjs',
    'spec.jsx': '_f_testjs',
    'spec.mjs': '_f_testjs',
    'test.ts': '_f_testts',
    'test.tsx': '_f_testts',
    'spec.ts': '_f_testts',
    'spec.tsx': '_f_testts',
    'e2e-test.ts': '_f_testts',
    'e2e-test.tsx': '_f_testts',
    'e2e-spec.ts': '_f_testts',
    'e2e-spec.tsx': '_f_testts',
    'texi': '_f_tex',
    'tikz': '_f_tex',
    'csv': '_f_csv',
    'todo': '_f_todo',
    'tt2': '_f_tt',
    'tsx': '_f_reactts',
    'd.ts': '_f_typescriptdef',
    'vala': '_f_vala',
    'vapi': '_f_vapi',
    'vash': '_f_vash',
    'vbhtml': '_f_vbhtml',
    'vbproj': '_f_vbproj',
    'vcxproj': '_f_vcxproj',
    'vsix': '_f_vscode',
    'vsixmanifest': '_f_vscode',
    'wasm': '_f_wasm',
    'doc': '_f_word',
    'docx': '_f_word',
    'docm': '_f_word',
    'dot': '_f_word',
    'dotx': '_f_word',
    'dotm': '_f_word',
    'wll': '_f_word',
    'wxml': '_f_wxml',
    'wxss': '_f_wxss',
    'xcodeproj': '_f_xcode',
    'xfl': '_f_xfl',
    'xib': '_f_xib',
    'xliff': '_f_xliff',
    'xlf': '_f_xliff',
    'pex': '_f_xml',
    'tmlanguage': '_f_xml',
    'zip': '_f_zip',
    'rar': '_f_zip',
    '7z': '_f_zip',
    'tar': '_f_zip',
    'gz': '_f_zip',
    'bzip2': '_f_zip',
    'xz': '_f_zip',
    'bz2': '_f_zip',
    'zipx': '_f_zip',
    'tgz': '_f_zip',
    'styl': '_f_stylus',
    'tmpl': '_f_tmpl',
    'mint': '_f_mint',
    'eex': '_f_eex',
    'conf': '_f_conf',
    'cfg.dist': '_f_conf',
    'cfg': '_f_conf',
    'ini': '_f_conf',
    'c': '_f_c',
    'cpp': '_f_cpp',
    'hpp': '_f_hpp',
    'ps1': '_f_powershell',
    'psm1': '_f_powershellmodule',
    'psd1': '_f_powershelldata',
    'njk': '_f_njk',
    'blade.php': '_f_blade',
    'blade': '_f_blade',
    'gql': '_f_graphql',
    'jepg': '_f_imagejpg',
    'webp': '_f_imagewebp',
    'jinja': '_f_jinja',
    'controller.ts': '_f_nestjscontroller',
    'controller.js': '_f_nestjscontroller',
    'module.ts': '_f_nestjsmodule',
    'service.ts': '_f_nestjsservice',
    'service.js': '_f_nestjsservice',
    'guard.ts': '_f_nestjsguard',
    'guard.js': '_f_nestjsguard',
    'decorator.ts': '_f_nestjsdecorator',
    'decorator.js': '_f_nestjsdecorator',
    'filter.ts': '_f_nestjsfilter',
    'filter.js': '_f_nestjsfilter',
    'txt': '_f_txt',
    'swift': '_f_swift',
    'rs': '_f_rust',
    'less': '_f_less',
    'dart': '_f_dartlang',
    'erl': '_f_erlang',
    'groovy': '_f_groovy',
    'java': '_f_java',
    'toml': '_f_toml',
    'elm': '_f_elm',
    'svelte': '_f_svelte',
    'go': '_f_go',
    'rb': '_f_ruby',
    'hbs': '_f_handlebars',
    'twig': '_f_twig',
    'bat': '_f_bat',
    'ex': '_f_elixir',
    'prisma': '_f_prisma',
    'jl': '_f_julia',
    'razor': '_f_razor',
    'cshtml': '_f_cshtml',
    'jsp': '_f_jsp',
    'res': '_f_rescript',
    'resi': '_f_rescript',
    'vert': '_f_opengl',
    'frag': '_f_opengl',
    'tesc': '_f_opengl',
    'tese': '_f_opengl',
    'geom': '_f_opengl',
    'comp': '_f_opengl',
    'glsl': '_f_opengl',
    'hlsl': '_f_opengl',
    'll': '_f_llvm',
    'bc': '_f_llvm',
    'info': '_f_info',
    'ipynb': '_f_ipynb',
    'sol': '_f_sol',
    'bicep': '_f_bicep',
    'pri': '_f_qt',
    'user': '_f_user',
    'ui': '_f_ui',
    'clj': '_f_clojure',
    'cljs': '_f_clojure',
    'cljc': '_f_clojure',
    'cljx': '_f_clojure',
    'edn': '_f_clojure',
    'tf': '_f_terraform',
    'tfvars': '_f_terraformvars',
    'ml': '_f_ocaml',
    'mli': '_f_ocamli',
    'mly': '_f_ocamly',
    'mll': '_f_ocamll',
    'fs': '_f_fsharp',
    'astro': '_f_astro',
    'godot': '_f_godot',
    'gd': '_f_godot',
    'tscn': '_f_tscn',
    'env': '_f_env',
    'hcl': '_f_hashicorp',
    'sentinel': '_f_sentinel',
    'feature': '_f_cucumber',
    'nix': '_f_nix',
    'edge': '_f_edge',
    'nvim': '_f_nvim',
    'tres': '_f_tres',
    'sln': '_f_sln',
    'suo': '_f_suo',
    'mesh': '_f_mesh',
    'gbl': '_f_gbl',
    'sql': '_f_sql',
    'v': '_f_v',
    'rego': '_f_rego',
    'scala': '_f_scala',
    'sc': '_f_scala',
    'd': '_f_d',
    'adoc': '_f_adoc',
    'mo': '_f_motoko',
    'patch': '_f_diff',
    'diff': '_f_diff',
  },
  fileNames: {
    'webpack.test.conf.ts': '_f_webpack',
    'webpack.test.conf.coffee': '_f_webpack',
    'webpack.test.conf.js': '_f_webpack',
    'webpack.rules.ts': '_f_webpack',
    'webpack.rules.coffee': '_f_webpack',
    'webpack.rules.js': '_f_webpack',
    'webpack.renderer.config.ts': '_f_webpack',
    'webpack.renderer.config.coffee': '_f_webpack',
    'webpack.renderer.config.js': '_f_webpack',
    'webpack.plugins.ts': '_f_webpack',
    'webpack.plugins.coffee': '_f_webpack',
    'webpack.plugins.js': '_f_webpack',
    'webpack.mix.ts': '_f_webpack',
    'webpack.mix.coffee': '_f_webpack',
    'webpack.mix.js': '_f_webpack',
    'webpack.main.config.ts': '_f_webpack',
    'webpack.main.config.coffee': '_f_webpack',
    'webpack.main.config.js': '_f_webpack',
    'webpack.prod.conf.ts': '_f_webpack',
    'webpack.prod.conf.coffee': '_f_webpack',
    'webpack.prod.conf.js': '_f_webpack',
    'webpack.prod.ts': '_f_webpack',
    'webpack.prod.coffee': '_f_webpack',
    'webpack.prod.js': '_f_webpack',
    'webpack.dev.conf.ts': '_f_webpack',
    'webpack.dev.conf.coffee': '_f_webpack',
    'webpack.dev.conf.js': '_f_webpack',
    'webpack.dev.ts': '_f_webpack',
    'webpack.dev.coffee': '_f_webpack',
    'webpack.dev.js': '_f_webpack',
    'webpack.config.production.babel.ts': '_f_webpack',
    'webpack.config.production.babel.coffee': '_f_webpack',
    'webpack.config.production.babel.js': '_f_webpack',
    'webpack.config.prod.babel.ts': '_f_webpack',
    'webpack.config.prod.babel.coffee': '_f_webpack',
    'webpack.config.prod.babel.js': '_f_webpack',
    'webpack.config.test.babel.ts': '_f_webpack',
    'webpack.config.test.babel.coffee': '_f_webpack',
    'webpack.config.test.babel.js': '_f_webpack',
    'webpack.config.staging.babel.ts': '_f_webpack',
    'webpack.config.staging.babel.coffee': '_f_webpack',
    'webpack.config.staging.babel.js': '_f_webpack',
    'webpack.config.development.babel.ts': '_f_webpack',
    'webpack.config.development.babel.coffee': '_f_webpack',
    'webpack.config.development.babel.js': '_f_webpack',
    'webpack.config.dev.babel.ts': '_f_webpack',
    'webpack.config.dev.babel.coffee': '_f_webpack',
    'webpack.config.dev.babel.js': '_f_webpack',
    'webpack.config.common.babel.ts': '_f_webpack',
    'webpack.config.common.babel.coffee': '_f_webpack',
    'webpack.config.common.babel.js': '_f_webpack',
    'webpack.config.base.babel.ts': '_f_webpack',
    'webpack.config.base.babel.coffee': '_f_webpack',
    'webpack.config.base.babel.js': '_f_webpack',
    'webpack.config.babel.ts': '_f_webpack',
    'webpack.config.babel.coffee': '_f_webpack',
    'webpack.config.babel.js': '_f_webpack',
    'webpack.config.production.ts': '_f_webpack',
    'webpack.config.production.coffee': '_f_webpack',
    'webpack.config.production.js': '_f_webpack',
    'webpack.config.prod.ts': '_f_webpack',
    'webpack.config.prod.coffee': '_f_webpack',
    'webpack.config.prod.js': '_f_webpack',
    'webpack.config.test.ts': '_f_webpack',
    'webpack.config.test.coffee': '_f_webpack',
    'webpack.config.test.js': '_f_webpack',
    'webpack.config.staging.ts': '_f_webpack',
    'webpack.config.staging.coffee': '_f_webpack',
    'webpack.config.staging.js': '_f_webpack',
    'webpack.config.development.ts': '_f_webpack',
    'webpack.config.development.coffee': '_f_webpack',
    'webpack.config.development.js': '_f_webpack',
    'webpack.config.dev.ts': '_f_webpack',
    'webpack.config.dev.coffee': '_f_webpack',
    'webpack.config.dev.js': '_f_webpack',
    'webpack.config.common.ts': '_f_webpack',
    'webpack.config.common.coffee': '_f_webpack',
    'webpack.config.common.js': '_f_webpack',
    'webpack.config.base.ts': '_f_webpack',
    'webpack.config.base.coffee': '_f_webpack',
    'webpack.config.base.js': '_f_webpack',
    'webpack.config.ts': '_f_webpack',
    'webpack.config.coffee': '_f_webpack',
    'webpack.config.js': '_f_webpack',
    'webpack.common.ts': '_f_webpack',
    'webpack.common.coffee': '_f_webpack',
    'webpack.common.js': '_f_webpack',
    'webpack.base.conf.ts': '_f_webpack',
    'webpack.base.conf.coffee': '_f_webpack',
    'webpack.base.conf.js': '_f_webpack',
    '.angular-cli.json': '_f_angular',
    'angular-cli.json': '_f_angular',
    'angular.json': '_f_angular',
    '.angular.json': '_f_angular',
    'api-extractor.json': '_f_api_extractor',
    'api-extractor-base.json': '_f_api_extractor',
    'appveyor.yml': '_f_appveyor',
    '.appveyor.yml': '_f_appveyor',
    'aurelia.json': '_f_aurelia',
    'azure-pipelines.yml': '_f_azurepipelines',
    '.vsts-ci.yml': '_f_azurepipelines',
    '.babelrc': '_f_babel',
    '.babelignore': '_f_babel',
    '.babelrc.js': '_f_babel',
    '.babelrc.cjs': '_f_babel',
    '.babelrc.mjs': '_f_babel',
    '.babelrc.json': '_f_babel',
    'babel.config.js': '_f_babel',
    'babel.config.cjs': '_f_babel',
    'babel.config.mjs': '_f_babel',
    'babel.config.json': '_f_babel',
    'vetur.config.js': '_f_vue',
    'vetur.config.ts': '_f_vue',
    '.bzrignore': '_f_bazaar',
    '.bazelrc': '_f_bazel',
    'bazel.rc': '_f_bazel',
    'bazel.bazelrc': '_f_bazel',
    'bitbucket-pipelines.yml': '_f_bitbucketpipeline',
    '.bithoundrc': '_f_bithound',
    '.bowerrc': '_f_bower',
    'bower.json': '_f_bower',
    '.browserslistrc': '_f_browserslist',
    'browserslist': '_f_browserslist',
    '.buckconfig': '_f_buckbuild',
    'gemfile': '_f_bundler',
    'gemfile.lock': '_f_bundler',
    'capacitor.config.json': '_f_capacitor',
    'cargo.toml': '_f_cargo',
    'cargo.lock': '_f_cargo',
    'chefignore': '_f_chef',
    'berksfile': '_f_chef',
    'berksfile.lock': '_f_chef',
    'policyfile': '_f_chef',
    'circle.yml': '_f_circleci',
    '.cfignore': '_f_cloudfoundry',
    '.codacy.yml': '_f_codacy',
    '.codacy.yaml': '_f_codacy',
    '.codeclimate.yml': '_f_codeclimate',
    'codecov.yml': '_f_codecov',
    '.codecov.yml': '_f_codecov',
    'config.codekit': '_f_codekit',
    'config.codekit2': '_f_codekit',
    'config.codekit3': '_f_codekit',
    '.config.codekit': '_f_codekit',
    '.config.codekit2': '_f_codekit',
    '.config.codekit3': '_f_codekit',
    'coffeelint.json': '_f_coffeelint',
    '.coffeelintignore': '_f_coffeelint',
    'commitlint.config.js': '_f_commitlint',
    'composer.json': '_f_composer',
    'composer.lock': '_f_composerlock',
    'conanfile.txt': '_f_conan',
    'conanfile.py': '_f_conan',
    '.condarc': '_f_conda',
    '.coveralls.yml': '_f_coveralls',
    'crowdin.yml': '_f_crowdin',
    '.csscomb.json': '_f_csscomb',
    '.csslintrc': '_f_csslint',
    '.cvsignore': '_f_cvs',
    '.boringignore': '_f_darcs',
    'dependabot.yml': '_f_dependabot',
    'dependencies.yml': '_f_dependencies',
    'devcontainer.json': '_f_devcontainer',
    'docker-compose.yml': '_f_docker',
    'docker-compose.ci-build.yml': '_f_docker',
    'docker-compose.override.yml': '_f_docker',
    'docker-compose.vs.debug.yml': '_f_docker',
    'docker-compose.vs.release.yml': '_f_docker',
    'docker-compose.debug.yml': '_f_dockerdebug',
    'docker-cloud.yml': '_f_docker',
    '.dockerignore': '_f_dockerignore',
    '.doczrc': '_f_docz',
    'docz.js': '_f_docz',
    'docz.json': '_f_docz',
    '.docz.js': '_f_docz',
    '.docz.json': '_f_docz',
    'doczrc.js': '_f_docz',
    'doczrc.json': '_f_docz',
    'docz.config.js': '_f_docz',
    'docz.config.json': '_f_docz',
    '.dojorc': '_f_dojo',
    '.drone.yml': '_f_drone',
    '.drone.yml.sig': '_f_drone',
    '.dvc': '_f_dvc',
    '.editorconfig': '_f_editorconfig',
    'elm-package.json': '_f_elm',
    '.ember-cli': '_f_ember',
    'emakefile': '_f_erlang',
    '.emakerfile': '_f_erlang',
    '.eslintrc': '_f_eslint',
    '.eslintignore': '_f_eslint',
    '.eslintcache': '_f_eslint',
    '.eslintrc.js': '_f_eslint',
    '.eslintrc.mjs': '_f_eslint',
    '.eslintrc.cjs': '_f_eslint',
    '.eslintrc.json': '_f_eslint',
    '.eslintrc.yaml': '_f_eslint',
    '.eslintrc.yml': '_f_eslint',
    'eslint-preset.js': '_f_eslint',
    'eslint.config.js': '_f_eslint',
    'eslint.config.cjs': '_f_eslint',
    'eslint.config.mjs': '_f_eslint',
    'eslint.config.ts': '_f_eslint',
    'app.json': '_f_expo',
    'app.config.js': '_f_expo',
    'app.config.json': '_f_expo',
    'app.config.json5': '_f_expo',
    'favicon.ico': '_f_favicon',
    '.firebaserc': '_f_firebase',
    'firebase.json': '_f_firebasehosting',
    'firestore.rules': '_f_firestore',
    'firestore.indexes.json': '_f_firestore',
    '.flooignore': '_f_floobits',
    '.flowconfig': '_f_flow',
    '.flutter-plugins': '_f_flutter',
    '.metadata': '_f_flutter',
    '.fossaignore': '_f_fossa',
    'ignore-glob': '_f_fossil',
    'fuse.js': '_f_fusebox',
    'gatsby-config.js': '_f_gatsby',
    'gatsby-config.ts': '_f_gatsby',
    'gatsby-node.js': '_f_gatsby',
    'gatsby-node.ts': '_f_gatsby',
    'gatsby-browser.js': '_f_gatsby',
    'gatsby-browser.ts': '_f_gatsby',
    'gatsby-ssr.js': '_f_gatsby',
    'gatsby-ssr.ts': '_f_gatsby',
    '.gitattributes': '_f_git',
    '.gitconfig': '_f_git',
    '.gitignore': '_f_git',
    '.gitmodules': '_f_git',
    '.gitkeep': '_f_git',
    '.mailmap': '_f_git',
    '.gitlab-ci.yml': '_f_gitlab',
    'glide.yml': '_f_glide',
    'go.sum': '_f_go_package',
    'go.mod': '_f_go_package',
    '.gqlconfig': '_f_graphql',
    '.graphqlconfig': '_f_graphql_config',
    '.graphqlconfig.yml': '_f_graphql_config',
    '.graphqlconfig.yaml': '_f_graphql_config',
    'greenkeeper.json': '_f_greenkeeper',
    'gridsome.config.js': '_f_gridsome',
    'gridsome.config.ts': '_f_gridsome',
    'gridsome.server.js': '_f_gridsome',
    'gridsome.server.ts': '_f_gridsome',
    'gridsome.client.js': '_f_gridsome',
    'gridsome.client.ts': '_f_gridsome',
    'gruntfile.js': '_f_grunt',
    'gruntfile.coffee': '_f_grunt',
    'gruntfile.ts': '_f_grunt',
    'gruntfile.babel.js': '_f_grunt',
    'gruntfile.babel.coffee': '_f_grunt',
    'gruntfile.babel.ts': '_f_grunt',
    'gulpfile.js': '_f_gulp',
    'gulpfile.coffee': '_f_gulp',
    'gulpfile.ts': '_f_gulp',
    'gulpfile.esm.js': '_f_gulp',
    'gulpfile.esm.coffee': '_f_gulp',
    'gulpfile.esm.ts': '_f_gulp',
    'gulpfile.babel.js': '_f_gulp',
    'gulpfile.babel.coffee': '_f_gulp',
    'gulpfile.babel.ts': '_f_gulp',
    'haxelib.json': '_f_haxe',
    'checkstyle.json': '_f_haxecheckstyle',
    '.p4ignore': '_f_helix',
    '.htmlhintrc': '_f_htmlhint',
    '.huskyrc': '_f_husky',
    'husky.config.js': '_f_husky',
    '.huskyrc.js': '_f_husky',
    '.huskyrc.json': '_f_husky',
    '.huskyrc.yaml': '_f_husky',
    '.huskyrc.yml': '_f_husky',
    'ionic.project': '_f_ionic',
    'ionic.config.json': '_f_ionic',
    'jakefile': '_f_jake',
    'jakefile.js': '_f_jake',
    'jest.config.json': '_f_jest',
    'jest.json': '_f_jest',
    '.jestrc': '_f_jest',
    '.jestrc.js': '_f_jest',
    '.jestrc.json': '_f_jest',
    'jest.config.js': '_f_jest',
    'jest.config.cjs': '_f_jest',
    'jest.config.mjs': '_f_jest',
    'jest.config.babel.js': '_f_jest',
    'jest.config.babel.cjs': '_f_jest',
    'jest.config.babel.mjs': '_f_jest',
    'jest.preset.js': '_f_jest',
    'jest.preset.ts': '_f_jest',
    'jest.preset.cjs': '_f_jest',
    'jest.preset.mjs': '_f_jest',
    '.jpmignore': '_f_jpm',
    '.jsbeautifyrc': '_f_jsbeautify',
    'jsbeautifyrc': '_f_jsbeautify',
    '.jsbeautify': '_f_jsbeautify',
    'jsbeautify': '_f_jsbeautify',
    'jsconfig.json': '_f_jsconfig',
    '.jscpd.json': '_f_jscpd',
    'jscpd-report.xml': '_f_jscpd',
    'jscpd-report.json': '_f_jscpd',
    'jscpd-report.html': '_f_jscpd',
    '.jshintrc': '_f_jshint',
    '.jshintignore': '_f_jshint',
    'karma.conf.js': '_f_karma',
    'karma.conf.coffee': '_f_karma',
    'karma.conf.ts': '_f_karma',
    '.kitchen.yml': '_f_kitchenci',
    'kitchen.yml': '_f_kitchenci',
    '.kiteignore': '_f_kite',
    'layout.html': '_f_layout',
    'layout.htm': '_f_layout',
    'lerna.json': '_f_lerna',
    'license': '_f_license',
    'licence': '_f_license',
    'license.md': '_f_license',
    'license.txt': '_f_license',
    'licence.md': '_f_license',
    'licence.txt': '_f_license',
    '.lighthouserc.js': '_f_lighthouse',
    '.lighthouserc.json': '_f_lighthouse',
    '.lighthouserc.yaml': '_f_lighthouse',
    '.lighthouserc.yml': '_f_lighthouse',
    'include.xml': '_f_lime',
    '.lintstagedrc': '_f_lintstagedrc',
    'lint-staged.config.js': '_f_lintstagedrc',
    '.lintstagedrc.js': '_f_lintstagedrc',
    '.lintstagedrc.json': '_f_lintstagedrc',
    '.lintstagedrc.yaml': '_f_lintstagedrc',
    '.lintstagedrc.yml': '_f_lintstagedrc',
    'manifest': '_f_manifest',
    'manifest.bak': '_f_manifest_bak',
    'manifest.skip': '_f_manifest_skip',
    '.markdownlint.json': '_f_markdownlint',
    'maven.config': '_f_maven',
    'pom.xml': '_f_maven',
    'extensions.xml': '_f_maven',
    'settings.xml': '_f_maven',
    '.hgignore': '_f_mercurial',
    'mocha.opts': '_f_mocha',
    '.mocharc.js': '_f_mocha',
    '.mocharc.json': '_f_mocha',
    '.mocharc.jsonc': '_f_mocha',
    '.mocharc.yaml': '_f_mocha',
    '.mocharc.yml': '_f_mocha',
    'modernizr': '_f_modernizr',
    'modernizr.js': '_f_modernizr',
    'modernizrrc.js': '_f_modernizr',
    '.modernizr.js': '_f_modernizr',
    '.modernizrrc.js': '_f_modernizr',
    'moleculer.config.js': '_f_moleculer',
    'moleculer.config.json': '_f_moleculer',
    'moleculer.config.ts': '_f_moleculer',
    '.mtn-ignore': '_f_monotone',
    '.nest-cli.json': '_f_nestjs',
    'nest-cli.json': '_f_nestjs',
    'nestconfig.json': '_f_nestjs',
    '.nestconfig.json': '_f_nestjs',
    'netlify.toml': '_f_netlify',
    '_redirects': '_f_netlify',
    'ng-tailwind.js': '_f_ng_tailwind',
    'nginx.conf': '_f_nginx',
    'build.ninja': '_f_ninja',
    '.node-version': '_f_node',
    'nodemon.json': '_f_nodemon',
    '.npmignore': '_f_npm',
    '.npmrc': '_f_npm',
    'package.json': '_f_npm',
    'package-lock.json': '_f_npmlock',
    'npm-shrinkwrap.json': '_f_npm',
    '.nsrirc': '_f_nsri',
    '.nsriignore': '_f_nsri',
    'nsri.config.js': '_f_nsri',
    '.nsrirc.js': '_f_nsri',
    '.nsrirc.json': '_f_nsri',
    '.nsrirc.yaml': '_f_nsri',
    '.nsrirc.yml': '_f_nsri',
    '.integrity.json': '_f_nsri-integrity',
    'nuxt.config.js': '_f_nuxt',
    'nuxt.config.ts': '_f_nuxt',
    '.nycrc': '_f_nyc',
    '.nycrc.json': '_f_nyc',
    '.merlin': '_f_ocaml',
    'paket.dependencies': '_f_paket',
    'paket.lock': '_f_paket',
    'paket.references': '_f_paket',
    'paket.template': '_f_paket',
    'paket.local': '_f_paket',
    '.php_cs': '_f_phpcsfixer',
    '.php_cs.dist': '_f_phpcsfixer',
    'phpunit': '_f_phpunit',
    'phpunit.xml': '_f_phpunit',
    'phpunit.xml.dist': '_f_phpunit',
    '.phraseapp.yml': '_f_phraseapp',
    'pipfile': '_f_pip',
    'pipfile.lock': '_f_pip',
    'platformio.ini': '_f_platformio',
    'pnpmfile.js': '_f_pnpm',
    'pnpm-workspace.yaml': '_f_pnpm',
    '.postcssrc': '_f_postcssconfig',
    '.postcssrc.json': '_f_postcssconfig',
    '.postcssrc.yml': '_f_postcssconfig',
    '.postcssrc.js': '_f_postcssconfig',
    '.postcssrc.cjs': '_f_postcssconfig',
    'postcss.config.js': '_f_postcssconfig',
    'postcss.config.cjs': '_f_postcssconfig',
    '.pre-commit-config.yaml': '_f_precommit',
    '.prettierrc': '_f_prettier',
    '.prettierignore': '_f_prettierignore',
    'prettier.config.js': '_f_prettier',
    'prettier.config.ts': '_f_prettier',
    'prettier.config.coffee': '_f_prettier',
    '.prettierrc.js': '_f_prettier',
    '.prettierrc.json': '_f_prettier',
    '.prettierrc.yml': '_f_prettier',
    '.prettierrc.yaml': '_f_prettier',
    'procfile': '_f_procfile',
    'protractor.conf.js': '_f_protractor',
    'protractor.conf.coffee': '_f_protractor',
    'protractor.conf.ts': '_f_protractor',
    '.jade-lintrc': '_f_pug',
    '.pug-lintrc': '_f_pug',
    '.jade-lint.json': '_f_pug',
    '.pug-lintrc.js': '_f_pug',
    '.pug-lintrc.json': '_f_pug',
    '.pyup': '_f_pyup',
    '.pyup.yml': '_f_pyup',
    'qmldir': '_f_qmldir',
    'quasar.conf.js': '_f_quasar',
    'rakefile': '_f_rake',
    'razzle.config.js': '_f_razzle',
    'readme.md': '_f_readme',
    '.rehyperc': '_f_rehype',
    '.rehypeignore': '_f_rehype',
    '.rehyperc.js': '_f_rehype',
    '.rehyperc.json': '_f_rehype',
    '.rehyperc.yml': '_f_rehype',
    '.rehyperc.yaml': '_f_rehype',
    '.remarkrc': '_f_remark',
    '.remarkignore': '_f_remark',
    '.remarkrc.js': '_f_remark',
    '.remarkrc.json': '_f_remark',
    '.remarkrc.yml': '_f_remark',
    '.remarkrc.yaml': '_f_remark',
    '.renovaterc': '_f_renovate',
    'renovate.json': '_f_renovate',
    '.renovaterc.json': '_f_renovate',
    '.retextrc': '_f_retext',
    '.retextignore': '_f_retext',
    '.retextrc.js': '_f_retext',
    '.retextrc.json': '_f_retext',
    '.retextrc.yml': '_f_retext',
    '.retextrc.yaml': '_f_retext',
    'robots.txt': '_f_robots',
    'rollup.config.js': '_f_rollup',
    'rollup.config.mjs': '_f_rollup',
    'rollup.config.coffee': '_f_rollup',
    'rollup.config.ts': '_f_rollup',
    'rollup.config.common.js': '_f_rollup',
    'rollup.config.common.mjs': '_f_rollup',
    'rollup.config.common.coffee': '_f_rollup',
    'rollup.config.common.ts': '_f_rollup',
    'rollup.config.dev.js': '_f_rollup',
    'rollup.config.dev.mjs': '_f_rollup',
    'rollup.config.dev.coffee': '_f_rollup',
    'rollup.config.dev.ts': '_f_rollup',
    'rollup.config.prod.js': '_f_rollup',
    'rollup.config.prod.mjs': '_f_rollup',
    'rollup.config.prod.coffee': '_f_rollup',
    'rollup.config.prod.ts': '_f_rollup',
    '.rspec': '_f_rspec',
    '.rubocop.yml': '_f_rubocop',
    '.rubocop_todo.yml': '_f_rubocop',
    'rust-toolchain': '_f_rust_toolchain',
    '.sailsrc': '_f_sails',
    '.sentryclirc': '_f_sentry',
    'serverless.yml': '_f_serverless',
    'snapcraft.yaml': '_f_snapcraft',
    '.snyk': '_f_snyk',
    '.solidarity': '_f_solidarity',
    '.solidarity.json': '_f_solidarity',
    '.stylelintrc': '_f_stylelint',
    '.stylelintignore': '_f_stylelint',
    '.stylelintcache': '_f_stylelint',
    'stylelint.config.js': '_f_stylelint',
    'stylelint.config.json': '_f_stylelint',
    'stylelint.config.yaml': '_f_stylelint',
    'stylelint.config.yml': '_f_stylelint',
    'stylelint.config.ts': '_f_stylelint',
    '.stylelintrc.js': '_f_stylelint',
    '.stylelintrc.json': '_f_stylelint',
    '.stylelintrc.yaml': '_f_stylelint',
    '.stylelintrc.yml': '_f_stylelint',
    '.stylelintrc.ts': '_f_stylelint',
    '.stylish-haskell.yaml': '_f_stylish_haskell',
    '.svnignore': '_f_subversion',
    'package.pins': '_f_swift',
    'symfony.lock': '_f_symfony',
    'windi.config.ts': '_f_windi',
    'windi.config.js': '_f_windi',
    'tailwind.js': '_f_tailwind',
    'tailwind.coffee': '_f_tailwind',
    'tailwind.ts': '_f_tailwind',
    'tailwind.config.js': '_f_tailwind',
    'tailwind.config.cjs': '_f_tailwind',
    'tailwind.config.coffee': '_f_tailwind',
    'tailwind.config.ts': '_f_tailwind',
    '.testcaferc.json': '_f_testcafe',
    '.tfignore': '_f_tfs',
    'tox.ini': '_f_tox',
    '.travis.yml': '_f_travis',
    'tsconfig.json': '_f_tsconfig',
    'tsconfig.app.json': '_f_tsconfig',
    'tsconfig.base.json': '_f_tsconfig',
    'tsconfig.common.json': '_f_tsconfig',
    'tsconfig.dev.json': '_f_tsconfig',
    'tsconfig.development.json': '_f_tsconfig',
    'tsconfig.e2e.json': '_f_tsconfig',
    'tsconfig.prod.json': '_f_tsconfig',
    'tsconfig.production.json': '_f_tsconfig',
    'tsconfig.server.json': '_f_tsconfig',
    'tsconfig.spec.json': '_f_tsconfig',
    'tsconfig.staging.json': '_f_tsconfig',
    'tsconfig.test.json': '_f_tsconfig',
    'tslint.json': '_f_tslint',
    'tslint.yaml': '_f_tslint',
    'tslint.yml': '_f_tslint',
    '.unibeautifyrc': '_f_unibeautify',
    'unibeautify.config.js': '_f_unibeautify',
    '.unibeautifyrc.js': '_f_unibeautify',
    '.unibeautifyrc.json': '_f_unibeautify',
    '.unibeautifyrc.yaml': '_f_unibeautify',
    '.unibeautifyrc.yml': '_f_unibeautify',
    'vagrantfile': '_f_vagrant',
    '.vimrc': '_f_vim',
    '.gvimrc': '_f_vim',
    '.vscodeignore': '_f_vscode',
    'tasks.json': '_f_vscode',
    'vscodeignore.json': '_f_vscode',
    '.vuerc': '_f_vueconfig',
    'vue.config.js': '_f_vueconfig',
    'wallaby.json': '_f_wallaby',
    'wallaby.js': '_f_wallaby',
    'wallaby.ts': '_f_wallaby',
    'wallaby.coffee': '_f_wallaby',
    'wallaby.conf.json': '_f_wallaby',
    'wallaby.conf.js': '_f_wallaby',
    'wallaby.conf.ts': '_f_wallaby',
    'wallaby.conf.coffee': '_f_wallaby',
    '.wallaby.json': '_f_wallaby',
    '.wallaby.js': '_f_wallaby',
    '.wallaby.ts': '_f_wallaby',
    '.wallaby.coffee': '_f_wallaby',
    '.wallaby.conf.json': '_f_wallaby',
    '.wallaby.conf.js': '_f_wallaby',
    '.wallaby.conf.ts': '_f_wallaby',
    '.wallaby.conf.coffee': '_f_wallaby',
    '.watchmanconfig': '_f_watchmanconfig',
    'wercker.yml': '_f_wercker',
    'wpml-config.xml': '_f_wpml',
    '.yamllint': '_f_yamllint',
    '.yaspellerrc': '_f_yandex',
    '.yaspeller.json': '_f_yandex',
    'yarn.lock': '_f_yarn',
    '.yarnrc': '_f_yarn',
    '.yarnclean': '_f_yarn',
    '.yarn-integrity': '_f_yarn',
    '.yarn-metadata.json': '_f_yarn',
    '.yarnignore': '_f_yarn',
    'yarn-error.log': '_f_yarnerror',
    '.yo-rc.json': '_f_yeoman',
    'now.json': '_f_vercel',
    '.nowignore': '_f_vercel',
    'vercel.json': '_f_vercel',
    '.vercelignore': '_f_vercel',
    'vite.config.js': '_f_vite',
    'vite.config.ts': '_f_vite',
    '.nvmrc': '_f_nvm',
    '.env.staging': '_f_env',
    '.env.preprod': '_f_env',
    '.env.prod': '_f_env',
    '.env.production': '_f_env',
    '.env.local': '_f_env',
    '.env.dev': '_f_env',
    '.env.development': '_f_env',
    '.env.example': '_f_env',
    '.env.test': '_f_env',
    '.jinja': '_f_jinja',
    'jenkins.yaml': '_f_jenkins',
    'jenkins.yml': '_f_jenkins',
    '.compodocrc': '_f_compodoc',
    '.compodocrc.json': '_f_compodoc',
    '.compodocrc.yaml': '_f_compodoc',
    '.compodocrc.yml': '_f_compodoc',
    'bsconfig.json': '_f_bsconfig',
    '.clang-format': '_f_llvm',
    '.clang-tidy': '_f_llvm',
    '.parcelrc': '_f_parcel',
    'dune': '_f_dune',
    'dune-project': '_f_duneproject',
    '.adonisrc.json': '_f_adonis',
    'astro.config.mjs': '_f_astroconfig',
    'svelte.config.js': '_f_svelteconfig',
    'svelte.config.ts': '_f_svelteconfig',
    '.tool-versions': '_f_toolversions',
    'CMakeSettings.json': '_f_cmake',
    'CMakeLists.txt': '_f_cmake',
    'Cargo.toml': '_f_cargo',
    'Cargo.lock': '_f_cargolock',
    'pnpm-lock.yaml': '_f_pnpmlock',
    'tauri.conf.json': '_f_tauri',
    'tauri.linux.conf.json': '_f_tauri',
    'tauri.windows.conf.json': '_f_tauri',
    'tauri.macos.conf.json': '_f_tauri',
    'next.config.js': '_f_nextconfig',
    'next.config.ts': '_f_nextconfig',
    'nextron.config.js': '_f_nextron',
    'nextron.config.ts': '_f_nextron',
    'poetry.toml': '_f_poetry',
    'poetry.lock': '_f_poetrylock',
    'pyproject.toml': '_f_pyproject',
    'rustfmt.toml': '_f_rustfmt',
    '.rustfmt.toml': '_f_rustfmt',
    'cucumber.yml': '_f_cucumber',
    'cucumber.yaml': '_f_cucumber',
    'cucumber.js': '_f_cucumber',
    'cucumber.ts': '_f_cucumber',
    'cucumber.cjs': '_f_cucumber',
    'cucumber.mjs': '_f_cucumber',
    'cucumber.json': '_f_cucumber',
    'flake.lock': '_f_flakelock',
    'ace': '_f_ace',
    'ace-manifest.json': '_f_acemanifest',
    'knexfile.js': '_f_knex',
    'knexfile.ts': '_f_knex',
    'launch.json': '_f_launch',
    'redis.conf': '_f_redis',
    'sequelize.js': '_f_sequelize',
    'sequelize.ts': '_f_sequelize',
    'sequelize.cjs': '_f_sequelize',
    '.sequelizerc': '_f_sequelize',
    '.sequelizerc.js': '_f_sequelize',
    '.sequelizerc.json': '_f_sequelize',
    'cypress.json': '_f_cypress',
    'cypress.env.json': '_f_cypress',
    'cypress.config.js': '_f_cypress',
    'cypress.config.ts': '_f_cypress',
    'cypress.config.cjs': '_f_cypress',
    'playwright.config.ts': '_f_playright',
    'playwright.config.js': '_f_playright',
    'playwright.config.cjs': '_f_playright',
    'vitest.config.ts': '_f_vitest',
    'vitest.config.js': '_f_vitest',
    'vitest.config.cjs': '_f_vitest',
    'pubspec.lock': '_f_flutterlock',
    'pubspec.yaml': '_f_flutter',
    '.packages': '_f_flutterpackage',
    '.htaccess': '_f_htaccess',
    'nx.json': '_f_nx',
    'project.json': '_f_nx',
    'v.mod': '_f_vmod',
    'quasar.config.js': '_f_quasar',
    'quasar.config.ts': '_f_quasar',
    'quasar.config.cjs': '_f_quasar',
    'quasar.config.mjs': '_f_quasar',
  },
}

export default iconsSchema
